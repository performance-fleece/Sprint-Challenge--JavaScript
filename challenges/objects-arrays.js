...
const universities = [];
function listUniversities(list) {
  for (let i = 0; i < list.length; i++) {
    universities.push(list[i].university);
  }
  universities.sort();
}

listUniversities(graduates);
...
function contact(array) {
  for (let i = 0; i < array.length; i++) {
    let tempcontact = [array[i].first_name, array[i].email];
    contactInfo.push(tempcontact.join(" "));
  }
}
...
zooAnimals.forEach((index) => animalNames.push([`Name: ${index.animal_name}, Scientific: ${index.scientific_name}.`]));
...
const lowerCase = zooAnimals.map((index) => index.animal_name.toLowerCase());
